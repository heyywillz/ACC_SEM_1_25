...
function addToCart(index, card) {
  ...
  const price = parseFloat(
    card.querySelector("h5").textContent.replace("$", "")
  );
...
function updateCartUI() {
      ...
      (item, index) => `
        ...
        item.name
          ...
          <div style="flex-grow:1; margin-left:10px">
            ...
            <p style="margin:0;color:gray">$${item.price.toFixed(2)} x ${
        item.quantity
      }</p>
          </div>
          <div style="display:flex;gap:4px">
            <button onclick="changeQuantity(${index}, -1)" style="background-color: hsl(14, 86%, 42%); padding:4px 10px; border: none; color: #fff; cursor: pointer;">-</button>
  ...
  cartContainer.innerHTML = `
        ...
        <hr>
        <h4>Total: $${total}</h4>
        <button style="margin-top:1rem;padding:10px 16px;border:none;background-color:var(--Red);color:#fff;border-radius:50px;cursor:pointer">Confirm Order</button>
      `;
}
